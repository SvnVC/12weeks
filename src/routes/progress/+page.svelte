<script lang="ts">
  ...
  import { Card, Progressbar, Button, Badge, Helper } from 'flowbite-svelte';
  ...
  onMount(async () => {
    data = await loadData();
    activeGoals = data.goals.filter(goal => goal.status === 'active');
    activeGoals = data.goals;
    const earliestStart = Math.min(
      ...activeGoals.filter(g => g.startDate).map(g => new Date(g.startDate).getTime())
    );
    ...
    activeGoals.forEach(goal => {
      ...
      const diffDays = Math.floor(diffMs / (1000 * 60 * 60 * 24));
      const currentWeek = Math.min(Math.max(Math.ceil(diffDays / 7) - 1, 0), 11); // 0-11
  ...
  function getWeekProgress(goal: Goal, week: string) {
    ...
    return [{ total, done, percentage: total ? Math.round((done / total) * 100) : 0, tasks }];
  }
...
<main class="mx-auto p-6 min-h-screen">
  ...
  <p class="text-lg text-gray-600 mb-8 text-center">{data.vision}</p>
        ...
        <Card class="p-6  min-w-[350px] ">
          ...
          <div class="mb-6">
            ...
            <div class="grid grid-cols-12 gap-2 mt-2">
              {#each getWeeklyScores(goal) as { week, percentage, tasks } (week)}
                ...
                >
                  <span class="text-xs font-medium text-gray-600 block mb-1">W{week + 1}</span>
                  <Progressbar
                    progress={percentage}
          ...
          <div class="mb-6">
            ...
            {#each getWeekProgress(goal, (hoveredWeeks[goal.id] ?? currentWeeks[goal.id]).toString()) as { total, done, percentage, tasks }}
              ...
              {#if tasks.length > 0}
                <Progressbar progress={percentage} color="green" size="h-4" class="mt-2 mb-2" labelOutside="{done} / {total}" classLabelOutside="text-green"/>
                <div class=" space-y-2">
                  {#each tasks as task (task.id)}
                    <div class="flex justify-between items-center text-sm text-gray-600">
                      <span>{task.title || 'Unnamed'}</span>
                      <Badge color="gray">
                        {task.done.filter(Boolean).length} / {Number(task.frequency)} ({getTaskPercentage(task)}%)
                      </Badge>
                    </div>
                  {/each}
                </div>
  ...
  {:else}
    <p class="text-center text-gray-500 mt-12">No active goals found.</p>
  {/if}
</main>